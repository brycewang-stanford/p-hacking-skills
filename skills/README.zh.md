...
| `01-phack-taxonomy` | map | 25 种 p-hacking 策略及其模拟假阳性率：Stefan & Schönbrodt 的 12 种 + 13 种计量特有自由度 + 搜索过程层 |
...
| `09-search-procedures` | red | 真人怎么搜：首个显著即停、随机预算、贪心坐标下降、爬山；在空数据上重放同一过程，得到**该搜索方式**的假阳性率 |
...
| `06-phack-detection` | blue | 对一批结果做 p-curve 检验（Elliott–Kudrin–Wüthrich 单调性、LCM、聚束、caliper） |
